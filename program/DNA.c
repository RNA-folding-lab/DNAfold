/*This program developed by Ya-Zhou Shi & Zi-Chun Mu  (2022.2.15)
  ...
  named as dsDNA1.0-SA.c  (where SA means Simulated Annealing)
  Here is the new version dsDNA4.0 (2022.6.18 for sequence input)
$ 
$ gcc -Wall -o3 dsDNA4.0-SA.c -o dsDNA -lm
$ ./dsDNA <P.dat or ./dsDNA
*/
#include <stdio.h>
#include <math.h>
#include <stdlib.h>
...
#define total1 100000    //Max steps in Folding annealing (Structure predicton)
...
#define t_si 10000         //The output frequency of structure information
#define p_move_3nt 0.4     //The relative probality of move_3nt
...
float rgp,rep,pl3p,pl4p,pl0,pl0p,PBP,pbp,G,GG,t0,M,m;
...
float U,Umin,Up,Up0,du,uu,u,ulj,uulj,uc,uuc,ub,uub,ue,uue,ud,uud,uN,uuN,us,uus,uco,uuco,uN1,uuN1;
...
float e=0.26,A=-3.6,B0=-11.0,Beta=0.67,Beta1=0.0;
...
float kpcpc=1.3,dpcpc=2.5,kcpcp=4.0,dcpcp=-2.95,kcpcN=1.6,dcpcN=-1.3,kNcpc=0.8,dNcpc=0.9;  //Dihedral
...
void Set_Files (void) 
{
 printf("This program is designed for dsDNA. Please note the input\n");
...
{ 
   rep=0.0;rgp=0.0;   //para for average of Rg & Ree 
...
/***********Calculate energy & structure information (Ree, Rg and lp) of one conformation*******************/
    ...
    if (fmod(t,tprint)==0||t==1||t==10000)  {printf("%d %f %f %f %f %f(%f)\n",t,rgp,rep,U,Up,nm*1.0/(N0*100.),p_accept_ratio);}	                      
...
void MC_Each_Step()
{
   ...
   ulj=0.0;ub=0.0;ue=0.0;ud=0.0;uc=0.0;/*uN=0.0;us=0.0;uco=0.0;*/
   uulj=0.0;uub=0.0;uue=0.0;uud=0.0;uuN=0.0;uus=0.0;uuc=0.0;uuco=0.0;
...
void Metropolis(void)
{
   int i,j,jc,jc1;
   float p=0.0;
   u=ulj+uN+us+uc+(ub+ue+ud)*0.5963+uco;           //Energy of conf. before changed
   uu=uulj+uuN+uus+uuc+(uub+uue+uud)*0.5963+uuco;  //Energy of conf. after changed
...
//if(t==1315200) {printf("1. Here? %d %d %d %d %d %f\n",t,chain_i0,i0,i,j,ulj);} 
                     } 
                 }   
            }
         }
...
//if(t==1315200) {printf("2. Here? %d %d %d %d %d %f\n",t,chain_i0,i0,i,j,ulj);}  
              } 
            }         
       } 
  }
...
// Energy of one confromation
 void ENERGY()
 {
     ...
     Energy=1;
     ulj=0.0;ub=0.0;uc=0.0; U=0.0; bp=0; BP=0;
/***********************************/
     for(jc=1;jc<=Nchain;jc++)  {
         for(i=1;i<=L0_chain[jc];i++)  {
    ...
    U=ulj+ub+uN+us+uc+uco; l++;  
    if (U<=Umin) {Umin=U;}
    else {Umin=Umin;}
    Up0+=U; Up=Up0/(t/tenergy);
    fprintf(fp6,"%d %d %f %f %f %f %f %f %f %f %f\n",t,l,U,Up,Umin,ulj,ub,uN,us,uc,uco);  fflush(fp6);
...
{
    int   i,tk,j=0,jc;
    float dis0=0.0,Xc=0.0,Yc=0.0,Zc=0.0,Rg,end;
       for(jc=1;jc<=Nchain;jc++)  {
         for(i=1;i<=L0_chain[jc];i++)  {
             Xc=Xc+x[jc][i]; Yc=Yc+y[jc][i]; Zc=Zc+z[jc][i];  
             j++; 
         }
       }
       Xc=Xc/(j*1.); Yc=Yc/(j*1.); Zc=Zc/(j*1.);
       for(jc=1;jc<=Nchain;jc++)  {
         for(i=1;i<=L0_chain[jc];i++)  {
             dis0=dis0+(x[jc][i]-Xc)*(x[jc][i]-Xc)+(y[jc][i]-Yc)*(y[jc][i]-Yc)+(z[jc][i]-Zc)*(z[jc][i]-Zc);   
         }
       }
       Rg=sqrt(dis0/(j*1.));
        ...
        if(t==1) {rgp=Rg;rep=end;}      
        tk=t/t_si;
        rgp=(rgp*(tk-1)+Rg)/(tk);   rep=(rep*(tk-1)+end)/(tk); 
        fprintf(fp2,"%d %d %f %f %f %f %d\n",t,tk,Rg,end,rgp,rep,nm); fflush(fp2);
 }
...
void FOLD(int i0)
{
 int   i,i_1,j,i0_1,i0_end; 
 float temp_step_angle=0.0,mv_x,mv_y,mv_z;
 float GaussianNum();
 for(j=1;j<=Nchain;j++)  {
    if (j!=chain_i0) { //keep the other chain fixed,only the chain_i0-th chain move;
        ...
        if (rand()/(RAND_MAX+1.)<p_move_3nt&&i0>=4&&i0<L0_chain[chain_i0]-12&&fmod(i0,3)!=0) {    //3nt fragment translation;
            step=step_3nt; i0_1=i0+1; i0_end=i0+8;      
            rand01=rand()/(RAND_MAX+1.);
        ...
        else if (rand()/(RAND_MAX+1.)<p_move_pivot_angle&&fmod(i0,3)!=0&&(i0>=7&&i0<=L0_chain[chain_i0]-6)) {//Pivot_angle fragment translation;     
            temp_step_angle=3.14*step_angle*GaussianNum();                             
            if(fmod(i0+2,3)==0) {i_1=i0+1;}    //ratate 3'End fragment
            if(fmod(i0+1,3)==0) {i_1=i0+2;}
            i0_1=i0+1; i0_end=L0_chain[j];
            for (i=1;i<=L0_chain[j];i++)  {   
                 if(i<=i0) {xx[j][i]=x[j][i];yy[j][i]=y[j][i];zz[j][i]=z[j][i];}
                 else  {Pivot_angle(j,i0,i_1,i,temp_step_angle);} 
            }
            Bonded_Pivot_angle(i0,j);
        }
        else if (rand()/(RAND_MAX+1.)<p_move_3nt&&fmod(i0+2,3)==0&&(i0>=4&&i0<=L0_chain[chain_i0]-12)) {   //fragment between Pi and Pi+9,rotate round Pi-Pi+9;  
            i_1=i0+9;  temp_step_angle=3.14*step_angle*GaussianNum(); i0_1=i0+1; i0_end=i0+8;
            for (i=1;i<=L0_chain[j];i++) {   
                if(i<=i0||i>=i0+9) {xx[j][i]=x[j][i];yy[j][i]=y[j][i];zz[j][i]=z[j][i];}
                else  {Pivot_angle(j,i0,i_1,i,temp_step_angle);} 
            }  
            Bonded_Rot_3nt(i0,j);  
        }
            ...
            if (fmod(i0,3)!=0)    {           //Backbone
                ...
                if (fmod(t,tran)==0) {
                    for(i=i0+1;i<=L0_chain[j];i++) {
                        xx[j][i]=xxx[i];yy[j][i]=yyy[i];zz[j][i]=zzz[i];} 
                }
                else                 { 
                     if (i0!=L0_chain[j])   {
